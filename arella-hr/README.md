...
| `EXPOSE_API_DOCS` | `true` in compose / `false` in the app | Swagger UI + OpenAPI schema at `/api/docs` — set `false` in production so the full API surface isn't advertised |
